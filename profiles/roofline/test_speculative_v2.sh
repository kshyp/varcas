...
echo "=========================================="
echo "TESTING: Speculative Decoding v2"
echo "Draft Model: JackFram/llama-160m"
echo "Target Model: TheBloke/Llama-2-7B-AWQ"
echo "=========================================="

# Start vLLM with speculative decoding using JSON config
SPEC_CONFIG='{"model": "JackFram/llama-160m", "num_speculative_tokens": 5}'
...
python -m vllm.entrypoints.openai.api_server \
  ...
  --enforce-eager \
  --port 8000 \
  --speculative-config "$SPEC_CONFIG" > varcas/profiles/roofline/speculative_results/server.log 2>&1 &
...
# Wait for ready
echo "Waiting for vLLM to be ready (downloading draft model if needed)..."
for i in {1..180}; do
