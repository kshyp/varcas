#!/bin/bash
# Profile vLLM server with NCU using attach mode
# This starts server normally, then attaches NCU for profiling during benchmark
# Usage: ./profile_vllm_with_attach.sh [profile_name]
...
echo "═══════════════════════════════════════════════════════════════"
echo "  NCU Attach Profiling: vLLM Server + $PROFILE_NAME Workload"
echo "═══════════════════════════════════════════════════════════════"
...
echo ""
echo "Step 1: Starting vLLM server (without NCU)..."
echo ""

# Start vLLM server WITHOUT NCU first (fast startup)
...
echo ""
echo "Step 3: Running warmup to initialize CUDA context..."
curl -s http://localhost:$VLLM_PORT/v1/completions \
    -H "Content-Type: application/json" \
    -d '{"model": "'"$MODEL"'", "prompt": "Hello world, this is a test", "max_tokens": 50}' > /dev/null 2>&1 || true
echo "✓ Warmup complete"
sleep 2

# Get the EngineCore process PID (the one doing GPU work)
ENGINE_PID=$(pgrep -f "EngineCore" | head -1)
if [ -z "$ENGINE_PID" ]; then
    echo "⚠ Engine process not found, using server PID: $SERVER_PID"
    ENGINE_PID=$SERVER_PID
else
    echo "✓ Found EngineCore PID: $ENGINE_PID"
fi

echo ""
echo "Step 4: Attaching NCU to PID $ENGINE_PID and running benchmark..."
echo ""

# Start NCU in attach mode in the background
ncu -f -o "$OUTPUT_FILE" \
    --attach $ENGINE_PID \
    --set basic &
...
echo "Running $PROFILE_NAME benchmark for ${BENCHMARK_DURATION}s..."
